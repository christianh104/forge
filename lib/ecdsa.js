...
ecdsa.generateKeyPair = function(options) {
  ...
  if (!(ecdsa.supportedCurves.includes(curveName))) {
...
ecdsa.publicKeyToAsn1 = function(key, options) {
  return key.toAsn1(options);
};

ecdsa.privateKeyToPem = function(key) {
  const msg = {
    type: 'PRIVATE KEY',
    body: asn1.toDer(ecdsa.privateKeyToAsn1(key)).getBytes()
  }
  return forge.pem.encode(msg)
};

ecdsa.privateKeyFromPem = function(bytes) {
  const der = forge.pem.decode(bytes)
  const secAsn1 = asn1.fromDer(der[0].body)
  return ECPrivateKey.fromAsn1(secAsn1);
};

ecdsa.publicKeyToPem = function(key) {
  const msg = {
    type: 'PUBLIC KEY',
    body: asn1.toDer(ecdsa.publicKeyToAsn1(key)).getBytes()
  }
  return forge.pem.encode(msg)
};

ecdsa.publicKeyFromPem = function(bytes) {
  const der = forge.pem.decode(bytes)
  const secAsn1 = asn1.fromDer(der[0].body)
  return ECPublicKey.fromAsn1(secAsn1);
};

ecdsa.ECPublicKey = ECPublicKey = function(ec, publicKey) {
  this._ec = ec;
  this._publicKey = publicKey;
  this.keyType = forge.oids.ecPublicKey;
};
...
ecdsa.ECPrivateKey = ECPrivateKey = function(ec, privateKey) {
  ...
  this.keyType = forge.oids[this._ec.curveName];
};
